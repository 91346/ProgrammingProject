...
function articleGet() {
    ...
    fetch(url + ArticleID, {                // UserID as a path parameter
        ...
        if (response.hasOwnProperty("Error")) {         //checks if response from server has an "Error"
            ...
            document.getElementById("DisplayArticle").innerHTML = response.Title + " " + response.Description + " " + response.Topic + " " + response.Author + " " + response.Date + " " + response.Picture + " " + response.Article;  //output data
        }
    });
}
